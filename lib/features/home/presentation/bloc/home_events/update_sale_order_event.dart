...
class UpdateSaleOrderEvent extends HomeEvent {
  final Map<String, dynamic> data;

  UpdateSaleOrderEvent({required this.data});

  @override
  Future<HomeState> failureOrResultState() async {
    final result = await di.sl<UpdateSaleOrderUseCase>().call(data);
    return await result.fold(
      (failure) async => UpdateSaleOrderFailureState(errorMessage: failure.message),
      (success) async {
        try {
        ...
        return UpdateSaleOrderSuccessState(response: success);
      },
    );
  }

  @override
  HomeState loaderState() => UpdateSaleOrderLoadingState();
}
